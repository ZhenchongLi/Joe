...
更深一层的"认输"发生在架构层面。vanilla RNN 怎么调参都记不远——Hochreiter 1991 年在博士论文里数学证明了这一点：连乘结构下，梯度必然指数衰减，调参无解。证完之后他才去设计 LSTM。

LSTM → Attention → Transformer 这条线，每一步都是在认输：上一个架构的天花板碰到了，继续调参回报趋近于零，该换一条路了。认输的价值在于它告诉你瓶颈在哪一层——参数问题调参能解决，架构问题要换架构。
